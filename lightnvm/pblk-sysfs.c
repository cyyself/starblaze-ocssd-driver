...
static ssize_t pblk_sysfs_rate_limiter(struct pblk *pblk, char *page)
{
	...
	return snprintf(page, PAGE_SIZE,
		"ringbuffer: user:%u/%u, gc:%u/%u(state=%u) (stop:<%u,full:>%u, free: %d/u:%d/t:%d)-%d\n",
				rb_user_cnt,
				rb_user_max,
				rb_gc_cnt,
				rb_gc_max,
...
static ssize_t pblk_sysfs_lines(struct pblk *pblk, char *page)
{
	...
	sz += snprintf(page + sz, PAGE_SIZE - sz,
		"lines: data:%d,log:%d-f:%d, meta:%d/%d, closed_lines:%d, bad:%d, corrupt:%d(dat:%d,log:%d), t:%d\n",
					cur_data, cur_log,
					nr_free_lines,
	...
	sz += snprintf(page + sz, PAGE_SIZE - sz,
		"data [%d] cur_sec:%d, left:%d, valid_secs:%d, sec_in_line:%d, map:%d/%d (inflight_io:%d)\n",
...
{
	int sz = 0;
	sz += snprintf(page + sz, PAGE_SIZE - sz, "inflight_writes: %lu\n", atomic_long_read(&pblk->inflight_writes));
	sz += snprintf(page + sz, PAGE_SIZE - sz, "inflight_reads: %lu\n", atomic_long_read(&pblk->inflight_reads));
	sz += snprintf(page + sz, PAGE_SIZE - sz, "req_writes: %lu\n", atomic_long_read(&pblk->req_writes));
	sz += snprintf(page + sz, PAGE_SIZE - sz, "nr_flush: %lu\n", atomic_long_read(&pblk->nr_flush));
	sz += snprintf(page + sz, PAGE_SIZE - sz, "padded_writes: %lu\n", atomic_long_read(&pblk->padded_writes));
	sz += snprintf(page + sz, PAGE_SIZE - sz, "padded_wb: %lu\n", atomic_long_read(&pblk->padded_wb));
	sz += snprintf(page + sz, PAGE_SIZE - sz, "sub_writes: %lu\n", atomic_long_read(&pblk->sub_writes));
	sz += snprintf(page + sz, PAGE_SIZE - sz, "failed_writes: %lu\n", atomic_long_read(&pblk->recov_writes));
	sz += snprintf(page + sz, PAGE_SIZE - sz, "recov_gc_writes: %lu\n", atomic_long_read(&pblk->recov_gc_writes));
	sz += snprintf(page + sz, PAGE_SIZE - sz, "recov_gc_reads: %lu\n", atomic_long_read(&pblk->recov_gc_reads));
	sz += snprintf(page + sz, PAGE_SIZE - sz, "cache_reads: %lu\n", atomic_long_read(&pblk->cache_reads));
	sz += snprintf(page + sz, PAGE_SIZE - sz, "sync_writes: %lu\n", atomic_long_read(&pblk->sync_writes));
	sz += snprintf(page + sz, PAGE_SIZE - sz, "sync_reads: %lu\n", atomic_long_read(&pblk->sync_reads));
	return sz;
/*
	return snprintf(page, PAGE_SIZE,
		"if_w:%lu\tif_r%lu\t%ld\t%llu\t%ld\t%lu\t%lu\t%lu\t%lu\t%lu\t%lu\t%lu\t%lu\n",
			atomic_long_read(&pblk->inflight_writes),
			atomic_long_read(&pblk->inflight_reads),
			atomic_long_read(&pblk->req_writes),
...
static struct ppa_addr ppa_sysfs;
static uint64_t lba_sysfs;

static ssize_t pblk_sysfs_set_trans_map(struct pblk *pblk,
			const char *page, size_t len)
{
	size_t c_len;
	...
	if (kstrtoull(page, 0, &lba_sysfs))
		return -EINVAL;

	ppa_sysfs = pblk_trans_map_get(pblk, lba_sysfs);
	return len;
}

static ssize_t pblk_sysfs_get_trans_map(struct pblk *pblk, char *page)
{
	int sz = 0;

	sz += snprintf(page + sz, PAGE_SIZE - sz, "lba=%lld, ", lba_sysfs);

	if (ppa_sysfs.c.is_cached) {
		sz += snprintf(page + sz, PAGE_SIZE - sz, "ppa(0x%016llx) cache line: %llu\n",
				ppa_sysfs.ppa, (u64)ppa_sysfs.c.line);
	} else {
		sz += snprintf(page + sz, PAGE_SIZE - sz, "ppa(0x%016llx) ch:%d, lun:%d, blk:%d, pg:%d, pl:%d, sec:%d\n",
				ppa_sysfs.ppa,
				ppa_sysfs.g.ch, ppa_sysfs.g.lun, ppa_sysfs.g.blk,
				ppa_sysfs.g.pg, ppa_sysfs.g.pl, ppa_sysfs.g.sec);
	}
...
static struct attribute sys_trans_map = {
	.name = "trans_map",
	.mode = 0644,
};
...
#endif
	else if (strcmp(attr->name, "trans_map") == 0)
		return pblk_sysfs_get_trans_map(pblk, buf);
	return 0;
}
				...
				const char *buf, size_t len)
{
	...
	if (strcmp(attr->name, "gc_force") == 0)
		...
		return pblk_sysfs_set_trans_map(pblk, buf, len);
	return 0;
}

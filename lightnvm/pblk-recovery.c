...
int pblk_recov_check_emeta(struct pblk *pblk, struct line_emeta *emeta_buf)
{
	...
	//printk("ocssd[%s]: emeta true_crc=0x%08x, device_crc=0x%08x, id=%d\n", __func__, emeta_buf->crc, crc, emeta_buf->header.id);
...
static int pblk_recov_l2p_from_emeta(struct pblk *pblk, struct pblk_line *line)
{
	...
	data_start = pblk_line_smeta_start(pblk, line) + smeta_sec;
			      ...
			      int left_ppas)
{
	...
	int i, j;
	int ret = 0;
	int min_write_pgs = line_get_min_write_pgs(line);

	printk("ocssd[%s]: left_ppas=%d, line_id=%d\n", __func__, left_ppas, line->id);
	spin_lock(&line->lock);
...
next_pad_rq:
	rq_ppas = pblk_calc_secs_line(pblk, left_ppas, 0, min_write_pgs);
	if (rq_ppas < min_write_pgs) {
	...
	meta_list = pblk_dev_dma_alloc(dev->parent, GFP_KERNEL, &dma_meta_list);
	if (!meta_list) {
		ret = -ENOMEM;
		goto fail_free_pad;
	}
	...
	for (i = 0; i < rqd->nr_ppas; ) {
		...
		w_ptr = pblk_alloc_page(pblk, line, min_write_pgs);
		...
		for (j = 0; j < min_write_pgs; j++, i++, w_ptr++) {
...
fail_free_bio:
	bio_put(bio);
fail_free_meta:
	pblk_dev_dma_free(dev->parent, meta_list, dma_meta_list);
fail_free_pad:
	kfree(pad_rq);
	vfree(data);
	return ret;
...
next_rq:
	...
	//print_ppa(geo, &rqd->ppa_list[0], "scan_all_oob", rqd->nr_ppas);
	ret = pblk_submit_io_sync(pblk, rqd);
	if (ret) {
		pr_err("pblk: I/O submission failed: %d\n", ret);
		return ret;
...
next_rq:
	...
	rq_ppas = pblk_calc_secs(pblk, left_ppas, 0);
	//printk("ocssd[%s]: left_ppas=%d, rq_ppas=%d\n", __func__, left_ppas, rq_ppas);
	if (!rq_ppas)
		rq_ppas = min_write_pgs;
	rq_len = rq_ppas * geo->csecs;
	...
	for (i = 0; i < rqd->nr_ppas; ) {
		...
		paddr = pblk_alloc_page(pblk, line, min_write_pgs);
		...
		for (j = 0; j < min_write_pgs; j++, i++, paddr++) {
			rqd->ppa_list[i] = addr_to_gen_ppa(pblk, paddr, line->id);
			//print_ppa(&pblk->dev->geo, &rqd->ppa_list[i], "scan_oob", i);
		}
	}

	print_ppa(geo, &rqd->ppa_list[0], "scan_oob", rqd->nr_ppas);
	ret = pblk_submit_io_sync(pblk, rqd);
	if (ret) {
		pr_err("pblk: I/O submission failed: %d\n", ret);
		bio_put(bio);
...
next_rq:
	...
	print_ppa(geo, &rqd->ppa_list[0], "gc_scan_oob", rqd->nr_ppas);
	ret = pblk_submit_io_sync(pblk, rqd);
	if (ret) {
		pr_err("pblk: I/O submission failed: %d\n", ret);
		bio_put(bio);
	...
	//bookmark: update l2p map
	for (i = 0; i < rqd->nr_ppas; i++) {
		map_l2p[i] = le64_to_cpu(meta_list[i].lba);
	}
...
struct pblk_line *pblk_recov_l2p(struct pblk *pblk)
{
	...
	for (i = 0; i < l_mg->nr_lines; i++) {
		...
		//printk("ocssd[%s]: smeta true_crc=0x%08x, device_crc=0x%08x, id=%d\n", __func__, smeta_buf->crc, crc, line->id);
		...
		if (memcmp(pblk->instance_uuid, smeta_buf->header.uuid, 16)) {
			printk("pblk: ignore line %u due to uuid mismatch\n", i);
			continue;
		}
		...
		found_lines++;
		printk("ocssd[%s]: need recover data line=%d, seq:%llu\n", __func__, line->id, smeta_buf->seq_nr);
	}

	printk("ocssd[%s]: scan start_meta, found lines=%d\n", __func__, found_lines);
	if (!found_lines) {
	...
	list_for_each_entry_safe(line, tline, &recov_list, list) {
		...
		//printk("ocssd[%s]: read end_meta line_id=%d\n", __func__, line->id);
		...
		//printk("ocssd[%s]: check line=%d end_meta pass!\n", __func__, line->id);
next:
		if (pblk_line_is_full(line)) {
			...
			data_line = line;
			printk("ocssd[%s]: open_lines=%d, meta_line=%d, data_line=%d\n", __func__, open_lines, meta_line, data_line->id);
		}
	}

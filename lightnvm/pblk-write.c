				    ...
				    struct pblk_c_ctx *c_ctx)
{
	...
	for (i = 0; i < c_ctx->nr_valid; i++) {
		...
		if (flags & PBLK_FLUSH_ENTRY) {
			...
			printk("ocssd[%s]: pos=%d, w_ctx->flags=0x%x\n", __func__, pos, w_ctx->flags);
...
#if 0
static struct ppa_addr ppa_addr_add_one(struct nvm_geo *geo, struct ppa_addr addr)
{
	addr.g.sec ++;
    if(addr.g.sec == geo->ws_min) {
		addr.g.sec = 0;

		addr.g.pl ++;
		if(addr.g.pl == geo->num_pln) {
			addr.g.pl = 0;

			addr.g.pg++;
			if (addr.g.pg == geo->num_pg) {
				addr.g.pg = 0;
...
static void pblk_map_remaining(struct pblk *pblk, struct ppa_addr *ppa)
{
	...
	while (!done) {
		//print_ppa(geo, &map_ppa, "remaining", cnt++);
		...
		if (geo->version == NVM_OCSSD_SPEC_12) {
			//bookmark: 标记当前blk的信息为坏块
			map_ppa = ppa_addr_add_one(geo, map_ppa);
			//map_ppa.ppa++;
			if (map_ppa.g.blk != ppa->g.blk)
				done = 1;
		} else {
			map_ppa.m.sec++;
	...
	printk("ocssd[%s]: line_id=%d, set flag has_write_err\n", __func__, line->id);
}
#else
static void pblk_map_remaining(struct pblk *pblk, struct ppa_addr *ppas, uint16_t nr_ppas)
{
	struct pblk_line *line;
	int i;

	line = &pblk->lines[pblk_ppa_to_line(*ppas)];
	spin_lock(&line->lock);

	for(i=0; i<nr_ppas; i++) {
		u64 paddr = pblk_dev_ppa_to_line_addr(pblk, ppas[i]);
	...
	line->w_err_gc->has_write_err = 1;
	spin_unlock(&line->lock);
	printk("ocssd[%s]: line_id=%d, nr_ppas=%d, set flag has_write_err\n", __func__, line->id, nr_ppas);
}
#endif
...
static void pblk_submit_rec(struct work_struct *work)
{
	...
	//bookmark: 重新map ppa地址
	//printk("ocssd[%s]: rqd->nr_ppas=%d, blk=%d\n", __func__, rqd->nr_ppas, ppa_list->g.blk);
...
static void pblk_end_io_write_meta(struct nvm_rq *rqd)
{
	...
	if (rqd->error) {
		pblk_log_write_err(pblk, rqd);
		pr_err("pblk: metadata write failed. line_id=%d\n", line->id);
		line->w_err_gc->has_write_err = 1;
	}

	sync = atomic_add_return(rqd->nr_ppas, &emeta->sync);
	if (sync == emeta->nr_entries) {
		//printk("ocssd[%s]: call pblk_line_close_ws line_id=%d, sync=%d\n", __func__, line->id, sync);
		pblk_gen_run_ws(pblk, line, NULL, pblk_line_close_ws, GFP_ATOMIC, pblk->close_wq);
	}
...
int pblk_submit_meta_io(struct pblk *pblk, struct pblk_line *meta_line)
{
	...
	void *data;
	u64 paddr;
	int rq_ppas = line_get_min_write_pgs(meta_line);
	int id = meta_line->id;
	int rq_len;
	int i, j;
	int ret;
	int emeta_len;
	if(true == pblk_line_is_slc(pblk, meta_line->id))
	...
	for (i = 0; i < rqd->nr_ppas; ) {
		...
		for (j = 0; j < rq_ppas; j++, i++, paddr++) {
			...
			//print_ppa(&dev->geo, &rqd->ppa_list[i], "emeta", i);
		}
	}

	spin_lock(&l_mg->close_lock);
	emeta->mem += rq_len;
	if (emeta->mem >= emeta_len)
		list_del(&meta_line->list);
	spin_unlock(&l_mg->close_lock);

	pblk_down_page(pblk, rqd->ppa_list, rqd->nr_ppas);

	//print_ppa(&dev->geo, &rqd->ppa_list[0], "emeta_write", rqd->nr_ppas);
				       ...
				       struct nvm_rq *data_rqd)
{
	...
	//bookmark: 进行偏移写，避免相同lun写，现在是相同ch，不同lun写
	//TODO: 可改为不同ch，不同lun写
						    ...
						    struct nvm_rq *data_rqd)
{
	...
	//printk("ocssd[%s]: meta_line_id=%d, emeta->mem=%d, emeta_len[0]=%d\n", __func__, meta_line->id, meta_line->emeta->mem, emeta_len);
	if(true == pblk_line_is_slc(pblk, meta_line->id))
		emeta_len = lm->emeta_len[0]/NAND_TLC_STEP;
	else
		emeta_len = lm->emeta_len[0];

	if (meta_line->emeta->mem >= emeta_len) {
		spin_unlock(&l_mg->close_lock);
		return NULL;
	}
	spin_unlock(&l_mg->close_lock);
...
static int pblk_submit_io_set(struct pblk *pblk, struct nvm_rq *rqd)
{
	...
	//print_ppa(&pblk->dev->geo, &rqd->ppa_list[0], "submit_io_set", rqd->nr_ppas);
...
static int pblk_submit_write(struct pblk *pblk)
{
	...
	if (pblk_line_is_full(line)) {
		...
		//printk("ocssd[%s]: pblk_line(%d) is full\n", __func__, line->id);
		line = pblk_line_replace_data(pblk);
		WARN(line==NULL, "ocssd[%s]: replace_data line=NULL\n", __func__);
		if(line != NULL)
	...
	if (resubmit) {
		...
		printk("ocssd[%s]: resubmit secs_to_sync=%d\n", __func__, secs_to_sync);
		...
		if (!secs_avail) {
			//printk("ocssd[%s]: there are no sectors in the cache\n", __func__);
			return 1;
		}

		//bookmark: 此处时强制补dummy
		secs_to_flush = pblk_rb_flush_point_count(&pblk->rwb);
		if (!secs_to_flush && secs_avail < line_get_min_write_pgs(line))
			return 1;

		secs_to_sync = pblk_calc_secs_to_sync(pblk, secs_avail, secs_to_flush);
		if (secs_to_sync > pblk->max_write_pgs) {
			printk("pblk-error: bad buffer sync calculation\n");
			return 1;
		}
	...
	//bookmark: 决定要写多少数据
	//printk("ocssd[%s]: ring_pos=%ld, secs_avail=%d, secs_to_flush=%d, secs_to_sync=%d\n", __func__, pos, secs_avail, secs_to_flush, secs_to_sync);

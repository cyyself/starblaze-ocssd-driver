...
//bookmark: GC_TIME
#define GC_TIME_MSECS 1000 //1000
...
/*********************************************************/
//bookmark: OP_THRESHOLD
#define PBLK_DEFAULT_OP (7)
//#define PBLK_DEFAULT_OP (95)
#define NUMS_SLC_LINE (100)

//#define ENABLE_ASYNC_META

#define NAND_TLC_STEP (3)
#define NAND_SLC_STEP (1)
#define CONFIG_NVM_DEBUG
/*********************************************************/
...
struct pblk_gc {
	...
	struct list_head w_list; //GC write target list
	struct list_head r_list; //GC read target list
...
//bookmark: block lines over all luns
struct pblk_line {
	struct pblk *pblk;
...
struct pblk_line_mgmt {
	...
	struct list_head slc_list;	/* Full lines slc for GC */
	...
	struct list_head gc_slc_list;	/* Full lines slc for GC */
...
struct pblk {
	...
	atomic_t inflight_io;		/* General inflight I/O counter, 正在跑的io计数 */
	 ...
	 */
	unsigned char *trans_map; //bookmark: l2p map
...
int pblk_submit_read_single(struct pblk *pblk, struct bio *bio);
int pblk_submit_read_gc(struct pblk *pblk, struct pblk_gc_rq *gc_rq);
/*
 * pblk recovery
 */
int pblk_recov_l2p_from_oob(struct pblk *pblk, struct pblk_line *line);
int pblk_scan_l2p_from_oob(struct pblk *pblk, struct pblk_line *line, uint64_t *map_l2p);
...
#if 0
//bookmark: original code for slc
...
#else
//bookmark: star ocssd1.2 for tlc
//fix for 96K write unit,根据bit数计算可表示的长度
static inline int pow_ppa(int len)
{
	int i=0;
	int val = 1;
	if(len > 0) {
...
//line addr to ppa
//bookmark: when use SLC cache, need check line_id
static inline struct ppa_addr addr_to_gen_ppa(struct pblk *pblk, u64 paddr, u64 line_id)
{
	...
	struct ppa_addr ppa;
	//TODO slc par
	int nand_step = NAND_TLC_STEP;
	if(line_id >= pblk->slc_start_line && line_id <= pblk->slc_end_line) {
		nand_step = NAND_SLC_STEP;
	}

	if (geo->version == NVM_OCSSD_SPEC_12) {
		struct nvm_addrf_12 *ppaf = (struct nvm_addrf_12 *)&pblk->addrf;
		int ch_len = pow_ppa(ppaf->ch_len);
		int lun_len = pow_ppa(ppaf->lun_len);
		int sec_len = pow_ppa(ppaf->sec_len);
		int pln_len = pow_ppa(ppaf->pln_len);

		ppa.ppa = 0;
		ppa.g.ch  = paddr / (sec_len*pln_len*nand_step) % ch_len;
		ppa.g.lun = paddr / (sec_len*pln_len*ch_len*nand_step) % lun_len;
		ppa.g.blk = line_id;
		ppa.g.pg  = paddr / (sec_len*pln_len)%nand_step + nand_step*(paddr/(ch_len*lun_len*sec_len*pln_len*nand_step));
		ppa.g.pl  = paddr / sec_len % pln_len;
		ppa.g.sec = paddr % sec_len;
	}
...
static inline u64 pblk_dev_ppa_to_line_addr(struct pblk *pblk, struct ppa_addr p)
{
	...
	if (geo->version == NVM_OCSSD_SPEC_12) {
		...
		int lun_num = (p.g.lun * ch_len + p.g.ch + (p.g.pg/nand_step)*(ch_len*lun_len)) * (sec_len * pln_len * nand_step);
		int pg_num = (p.g.pg % nand_step) * (sec_len * pln_len);
		int pl_num = p.g.pl * sec_len;
		paddr = (lun_num+pg_num+pl_num+p.g.sec);
	}
				      ...
				      struct line_smeta *smeta)
{
	...
	if(true == pblk_line_is_slc(pblk, smeta->header.id)) {
				      ...
				      struct line_emeta *emeta)
{
	...
	if(true == pblk_line_is_slc(pblk, emeta->header.id)) {
			     ...
			     char *msg, int error)
{
	if (p->c.is_cached) {
		...
		pr_err("ppa(%s: %2d) ch:%d, lun:%d, blk:%d, pg:%d, pl:%d, sec:%d\n",
			msg, error,
...
static inline void pblk_print_failed_rqd(struct pblk *pblk, struct nvm_rq *rqd, int error)
{
	//TODO: need ppa_status
#if 0
	int bit = -1;
	...
	//for (i = 0; i < rqd->nr_ppas; i++) {
		//print_ppa(&pblk->dev->geo, &rqd->ppa_list[i], "failed_rqd", i);
	//}
	if (rqd->nr_ppas ==  1) {
		print_ppa(&pblk->dev->geo, &rqd->ppa_addr, "failed_rqd", rqd->nr_ppas);
	} else {
		print_ppa(&pblk->dev->geo, &rqd->ppa_list[0], "failed_rqd", rqd->nr_ppas);
	}
#endif

	pr_err("%s: error:0x%x, ppa_status:0x%llx\n", __func__, error, rqd->ppa_status);
}
...
static inline int pblk_check_io(struct pblk *pblk, struct nvm_rq *rqd)
{
	...
	//printk("ocssd[%s]: opcode=0x%02x, nr_ppas=%d, flags=%x\n", __func__, rqd->opcode, rqd->nr_ppas, rqd->flags);
...
#else
static inline void print_ppa(struct nvm_geo *geo, struct ppa_addr *p, char *msg, int error)
{
}
#endif
